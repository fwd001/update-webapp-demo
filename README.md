...
通过定时轮询打包产物中的 `version.json` 对比本地记录版本，检测到新版本时在页面内弹出提示，用户确认后刷新加载最新静态资源。零后端依赖、逻辑与框架无关（Vue/React 均可复用）。

## 实现思路

- 构建阶段在 `dist/` 写入 `version.json`：包含一个可唯一标识构建的 `version` 字段（时间戳或 git hash）。
- 运行时（仅生产）：基于 `useIntervalFn` 轮询 `publicPath/version.json`，结合页面可见性前台轮询、后台暂停，切回前台立即检查一次。
- 版本对比：将远端 `version` 与本地 `localStorage('app_version')` 比较；不同则触发全局 `window.__showUpdatePrompt(cb)` 显示更新提示；点击确认写入新版本并刷新页面。
...
> 写入 `version.json` 可在 Vite 的 `closeBundle`、CI/CD 或托管平台构建脚本中实现。
...
3) 生产构建后确保 `dist/version.json` 存在：
...
## 接入到 React 项目（React 18）

1) 复用检测逻辑与类型：复制本仓库的检测文件（`src/components/UpdatePopup/index.ts` 可重命名为 `update-check.ts`）与 `src/global.d.ts`。

2) 实现 React 弹窗组件：组件挂载时注册 `window.__showUpdatePrompt = (cb) => { ... }`，UI 自由实现。
...
其余与 Vue 一致：构建时写入 `version.json`，可选设置 `window.__bizConfig__`。
...
- 在 `vite.config.ts` 的 `closeBundle` 钩子或 CI/CD 构建脚本生成 `dist/version.json`：
...
- 参考“切换到 React 的接入指引”中的 `UpdateBanner.tsx` 最小示例；组件挂载时赋值 `window.__showUpdatePrompt = (cb) => { ... }`。
...
- 组件样式与布局可自由定制。
- 如果项目使用路由懒加载或多入口，不影响此方案；该方案仅依赖全局函数与轮询逻辑。
